...
function insereFotosProduto($id_produto, $files, $descricao, $id_usuario, PDO $conn = null)
{
	$conn = is_null($conn) ? Conexao::criarConexao() : $conn;
        ...
        $descricao = substr($descricao, 0, 27);
        try {
        ...
        if (isset($files['fotos'])) {
                ...
                for ($i = 0; $i < count($imagens['name']); $i++) {
                        if ($imagens['name'][$i] != "") {
                                ...
                                $img_extensao = array('.jpg', '.JPG', '.jpeg', '.JPEG');
                                ...
                                $text_length =  ( strlen($id_produto) * ($text_font_size * 0.78));
                                $img_texto = imagecreate($text_length + ($padding * 2), $text_font_size); //cria uma imagem
                                imagecolorallocate($img_texto,$background['tag']['red'],$background['tag']['green'],$background['tag']['blue']); // cor de fundo 
                                $textcolor = imagecolorallocate( $img_texto,$background['font']['red'],$background['font']['green'],$background['font']['blue']); //cor da font
                                imagettftext( $img_texto, $text_font_size, 0, $padding, $text_font_size, $textcolor, __DIR__ . '/../fonts/Swis721_BT_Bold.ttf', $id_produto); //cria a etiqueta com a descrição
                                ...
                                imagecopymerge( $imagem, $img_texto, $largura_calculada, $altura_calculada , 0, ($text_font_size * 0.78) - $text_font_size, $width_card , $text_font_size + $padding, $background['alpha']); //coloca a etiqueta sobre a imagem do produto
                                ...
                                $nomeimagem = PREFIXO_LOCAL ."small_". $id_produto . "_" . $sequencia . "_" . date('d_m_Y') . $extensao;
                                $largura_max        = 100;
                                $altura_max        = 100;
                                $temp_file         = $imagens['tmp_name'][$i];
                                upload($temp_file, $nomeimagem, $largura_max, $altura_max); // ajusta o tamanho da imagem e salva na pasta upload
