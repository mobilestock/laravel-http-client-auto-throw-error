...
CREATE TRIGGER `produtos_before_insert` BEFORE INSERT ON `produtos` FOR EACH ROW BEGIN
	...
	IF(NEW.valor_custo_produto > 0)THEN
        ...
        IF (VALOR_CALCULO_PORCENTAGEM_ < CUSTO_MAX_APLICAR_TAXA_ML) THEN
            SET NEW.valor_venda_ml = NEW.valor_venda_ml + TAXA_PRODUTO_BARATO_ML;
        END IF;

        IF (VALOR_CALCULO_PORCENTAGEM_ < CUSTO_MAX_APLICAR_TAXA_MS) THEN
            SET NEW.valor_venda_ms = NEW.valor_venda_ms + TAXA_PRODUTO_BARATO_MS;
        END IF;
	END IF;
END//
