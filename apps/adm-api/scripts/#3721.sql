...
CREATE TRIGGER `produtos_before_insert` BEFORE INSERT ON `produtos` FOR EACH ROW BEGIN
	...
	IF(NEW.valor_custo_produto > 0)THEN
        ...
        IF (VALOR_CALCULO_PORCENTAGEM_ < CUSTO_MAX_APLICAR_TAXA_ML AND NEW.id NOT IN (82044, 82042, 99265)) THEN
        ...
        IF (VALOR_CALCULO_PORCENTAGEM_ < CUSTO_MAX_APLICAR_TAXA_MS AND NEW.id NOT IN (82044, 82042, 99265)) THEN
