...
class Cript{
    ...
    public function criptInt($valor){  
        if(strlen($valor)>18){return 'erro - Tamanho limite int(11)'; exit;} 
        $vl = substr(date("s"),1,1) . self::invertnum(str_replace(".","2", substr(gettimeofday("s"),-1))) . self::invertnum(str_replace(".","2", substr(gettimeofday("s"),-2, -1)));
    
        if($valor < 1000){
            $valor = $valor * self::servernamber($vl);
            $tip   = "4";
        }
        ...
        if(isset($valor[1]))$valor[1]  =  self::invertnum($valor[1]);
        if(isset($valor[3]))$valor[3]  =  self::invertnum($valor[3]);
        if(isset($valor[7]))$valor[7]  =  self::invertnum($valor[7]);
        if(isset($valor[4]))$valor[4]  =  self::invertnum($valor[4]);
        if(isset($valor[6]))$valor[6]  =  self::invertnum($valor[6]);
        if(isset($valor[10]))$valor[10] =  self::invertnum($valor[10]);
        if(isset($valor[11]))$valor[11] =  self::invertnum($valor[11]);
        if(isset($valor[13]))$valor[13] =  self::invertnum($valor[13]);
        if(isset($valor[15]))$valor[15] =  self::invertnum($valor[15]);
        if(isset($valor[17]))$valor[17] =  self::invertnum($valor[17]);
        if(isset($valor[21]))$valor[21] =  self::invertnum($valor[21]);
        if(isset($valor[22]))$valor[22] =  self::invertnum($valor[22]);
        if(isset($valor[23]))$valor[23] =  self::invertnum($valor[23]);
        if(isset($valor[27]))$valor[27] =  self::invertnum($valor[27]);
        if(isset($valor[30]))$valor[30] =  self::invertnum($valor[30]);
        if(isset($valor[32]))$valor[32] =  self::invertnum($valor[32]);
        if(isset($valor[33]))$valor[33] =  self::invertnum($valor[33]);
    ...
    public function dcriptInt($valor, $erro){
