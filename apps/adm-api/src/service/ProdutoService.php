...
class ProdutoService
{
    ...
    public static function insereFotosProduto(int $id_produto, array $files, string $descricao): void
    {
        if (is_null(Auth::id())) {
        ...
        $sequencia = self::buscaSequenciaFotoProduto($id_produto);
        $sequencia++;
        ...
        if (isset($files['fotos'])) {
            ...
            for ($i = 0; $i < count($imagens['name']); $i++) {
                if ($imagens['name'][$i] != '') {
                    ...
                    self::inserirImagensProduto($id_produto, $caminhoImagens, $nomeFoto, $sequencia);
                    $sequencia++;
                    ...
                    $nomeFoto = $nomeimagem;
                    $caminhoImagens = 'https://cdn-s3.' . $_ENV['URL_CDN'] . '/' . $nomeimagem;
                    self::inserirImagensProduto($id_produto, $caminhoImagens, $nomeFoto, $sequencia, 3, 'SM');
                    $sequencia++;
                }
            }
        }
        ...
        if (isset($files['fotos_calcadas'])) {
            ...
            for ($i = 0; $i < count($imagens['name']); $i++) {
                if ($imagens['name'][$i] != '') {
                    ...
                    $nomeFoto = $nomeimagem;
                    $caminhoImagens = 'https://cdn-s3.' . $_ENV['URL_CDN'] . '/' . $nomeimagem;
                    self::inserirImagensProduto($id_produto, $caminhoImagens, $nomeFoto, $sequencia, 1, 'LG');
                    $sequencia++;
                }
            }
        }
            ...
            for ($i = 0; $i < count($imagens['name']); $i++) {
                if ($imagens['name'][$i] != '') {
                    ...
                    if (in_array($extensao, $img_extensao)) {
                        ...
                        $nomeFoto = $nomeimagem;
                        $caminhoImagens = 'https://cdn-s3.' . $_ENV['URL_CDN'] . '/' . $nomeimagem;
                        self::inserirImagensProduto($id_produto, $caminhoImagens, $nomeFoto, $sequencia, 2);
                        $sequencia++;
                    } else {
...
{
    $query = "SELECT MAX(produtos_foto.sequencia) `sequencia` FROM produtos_foto WHERE produtos_foto.id = :id_produto";
