...
namespace MobileStock\service\Adiantamento;

use Error;
use MobileStock\model\RegraPagamentoSeller\Emprestimo;
use PDO;

class EmprestimoService extends Emprestimo
{
    public function criaEmprestimo(PDO $conexao): int
    {
        $sql = 'INSERT INTO emprestimo (' . implode(',', array_keys(array_filter(get_object_vars($this)))) . ') VALUES (';
    ...
    public function buscaLancamentoAdiantamento(pdo $conexao, float $valor, $idCliente){
        $sql="SELECT lancamento_financeiro.id,lancamento_financeiro.id_prioridade_saque FROM lancamento_financeiro 
                    WHERE lancamento_financeiro.id_colaborador = $idCliente 
                    AND lancamento_financeiro.valor_total = $valor 
                    AND lancamento_financeiro.origem = 'EM' ORDER BY id DESC LIMIT 1";
        $stmt = $conexao->prepare($sql);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
public function atualizaAdiantamento(pdo $conexao)
    {
        $dados = [];
        ...
        $sql .= " " . implode(',', $dados) . " WHERE emprestimo.id = '" . $this->id. "'";
