...
class ConversorStrings
{
	...
	public static function trataStringPesquisaCatalogo(string $pesquisa): string
	{
		$pesquisa = mb_convert_case($pesquisa, MB_CASE_LOWER);
		...
		if (count($pesquisaComoArray) > 0) return implode('|', $pesquisaComoArray);
		return implode(' ', $pesquisaComoArray);
	}

	public function convertePrimeiraLetraMaiusculo(): array
	{
		return array_map(function ($possivelString) {
			if (is_string($possivelString) && !is_numeric($possivelString)) return ucfirst($possivelString);
	...
	{
		return implode(',', array_reduce($valores, function(array $total, array $item) {
