...
class TrocaPendenteRepository
{
    ...
    public static function buscaTrocasPendentesConfirmadas(PDO $conexao, int $id_cliente): array
    {
        $sql = $conexao->prepare(
            "SELECT
                ...
                COALESCE((
                    ...
                    WHERE EXISTS(
                        ...
                        WHERE entregas_devolucoes_item.uuid_produto = troca_pendente_item.uuid
                ...
                COALESCE((
                    SELECT lancamento_financeiro.valor
                    FROM lancamento_financeiro
                    WHERE lancamento_financeiro.numero_documento = troca_pendente_item.uuid
                        AND lancamento_financeiro.id_colaborador = troca_pendente_item.id_cliente
                        AND lancamento_financeiro.origem = 'TX'
                    LIMIT 1
                ), 0) taxa
            FROM troca_pendente_item
            INNER JOIN produtos on (produtos.id = troca_pendente_item.id_produto)
            INNER JOIN usuarios ON usuarios.id = troca_pendente_item.id_vendedor
            WHERE troca_pendente_item.id_cliente = :idCliente
            GROUP BY troca_pendente_item.uuid;"
        );
        $sql->bindValue(':idCliente', $id_cliente, PDO::PARAM_INT);
        $sql->execute();
