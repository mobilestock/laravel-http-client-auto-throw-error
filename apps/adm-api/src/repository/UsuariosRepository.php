...
class UsuariosRepository extends MobileStockBD
{
    ...
    public static function buscaDadosDashboardSeller(PDO $conexao, int $idCliente): ?array
    {
        $stmt = $conexao->prepare(
            "SELECT reputacao_fornecedores.media_envio dias_despacho,
                reputacao_fornecedores.taxa_cancelamento,
                COALESCE(reputacao_fornecedores.valor_vendido, 0) valor_vendido,
                reputacao_fornecedores.reputacao
            FROM reputacao_fornecedores
            WHERE reputacao_fornecedores.id_colaborador = :idCliente"
        );
        ...
        $requisitos = ProdutosPontosMetadadosService::buscaValoresMetadados($conexao, [
        ...
        $consulta['dias_impulsionar'] = (int) self::buscaDiasFaltaParaDesbloquearBotaoAtualizadaDataEntradaProdutos(
            $conexao,
            $idCliente
        )['dias'];

        $diasDespacho = (int) $consulta['dias_despacho'];
        if ($diasDespacho) {
            $progressoDiasDespacho = ($requisitos['MEDIA_DIAS_ENVIO_MELHOR_FABRICANTE'] / $diasDespacho) * 100;
            if ($progressoDiasDespacho > 100) {
                $progressoDiasDespacho = 100;
            }
        } else {
            $progressoDiasDespacho = 0;
        }

        if ($consulta['taxa_cancelamento']) {
            $consulta['taxa_cancelamento'] = (float) $consulta['taxa_cancelamento'];
            $progressoTaxaCancelamento =
                ($requisitos['TAXA_CANCELAMENTO_MELHOR_FABRICANTE'] / ($consulta['taxa_cancelamento'] ?: 1)) * 100;
            if ($progressoTaxaCancelamento > 100) {
                $progressoTaxaCancelamento = 100;
            }
        } else {
            $progressoTaxaCancelamento = 0;
        }

        $consulta['valor_vendido'] = (float) $consulta['valor_vendido'];
        if ($consulta['valor_vendido'] > 0) {
            $progressoValorVendido =
                ($consulta['valor_vendido'] / $requisitos['VALOR_VENDIDO_MELHOR_FABRICANTE']) * 100;
            if ($progressoValorVendido > 100) {
                $progressoValorVendido = 100;
            }
        } else {
            $progressoValorVendido = 0;
        }

        $consulta['objetivos'] = [
            'dias_despacho_concluido' => $progressoDiasDespacho === 100,
            'taxa_cancelamento_concluido' => $progressoTaxaCancelamento === 100,
            'valor_vendido_concluido' => $progressoValorVendido === 100,
        ];

        $consulta['porcentagem_barra'] = round(
            ($progressoDiasDespacho + $progressoTaxaCancelamento + $progressoValorVendido) / 3
        );
