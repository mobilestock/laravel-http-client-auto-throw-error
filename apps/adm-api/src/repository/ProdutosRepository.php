...
class ProdutosRepository
{
    ...
    /**
     * @deprecated
     * Usar PublicacoesPublic::catalogoPublicacoes()
     */
    public static function produtosCatalogoApiMeuLook(
        PDO $conexao,
        string $ordenar = '',
        int $idCliente = null,
        int $pagina = 1
        ...
        $filtroWhereSQL = '';
        $filtroOrderSQL = 'ORDER BY tem_estoque DESC ';

        // Se não estiver logado, não vê produtos especiais
        if (!$idCliente) {
            $filtroWhereSQL .= ' AND produtos.especial = 0 ';
        }

        // Ordenação de cada filtro
        if ($ordenar === 'lancamentos') {
            $filtroOrderSQL .= ' , produtos.id DESC';
        } elseif ($ordenar === 'menorPreco') {
            $filtroOrderSQL .= ' , produtos.valor_custo_produto ASC';
        } elseif ($ordenar === 'promocao') {
            $filtroOrderSQL .= ' , produtos.preco_promocao DESC';
        }
        $filtroOrderSQL .= ' , produtos.data_entrada DESC';

        // Paginação
        $porPagina = 100;
        $offset = ($pagina - 1) * $porPagina;

        // Auxiliares de retorno
        $consultaFoguinho = [];
        $retornoArray = [];

        // Subselect preço produto baseado no ponto
        $subselectPreco = 'produtos.valor_venda_ml valor';

        $selectFoto = "COALESCE(
            (SELECT caminho FROM produtos_foto WHERE produtos_foto.id = produtos.id AND tipo_foto = 'LG' LIMIT 1),
            (SELECT caminho FROM produtos_foto WHERE produtos_foto.id = produtos.id AND tipo_foto IN ('MD', 'XL') LIMIT 1))";

        if (($filtroWhereSQL === ' AND produtos.especial = 0 ' || $filtroWhereSQL === '') && !$ordenar) {
            if ($pagina == 1) {
                $query = "SELECT
                    produtos.preco_promocao > 0 situacao_Promocao,
                    produtos.preco_promocao = 0 situacao_Normal,
                    ...
                    produtos.preco_promocao promocao,
                    CONCAT(GROUP_CONCAT(DISTINCT estoque_grade.nome_tamanho)) estoque,
                    IF(SUM(estoque_grade.estoque) > 0,1,0) tem_estoque,
                    EXISTS((SELECT 1 FROM ranking_produtos_meulook WHERE ranking_produtos_meulook.id_produto = produtos.id LIMIT 1)) foguinho,
                    $subselectPreco
                FROM produtos
                INNER JOIN estoque_grade ON estoque_grade.id_produto = produtos.id
                WHERE produtos.posicao_acessado > 0 AND estoque_grade.estoque > 0 {$filtroWhereSQL}
                GROUP BY produtos.id
                ORDER BY produtos.posicao_acessado DESC;";

                $consultaFoguinho = $conexao->query($query)->fetchAll(PDO::FETCH_ASSOC);
                $filtroWhereSQL .= ' AND produtos.posicao_acessado < 1';
            }
            ...
            $query = "SELECT
                ...
                $subselectPreco
            FROM produtos
            INNER JOIN estoque_grade ON estoque_grade.id_produto = produtos.id AND estoque_grade.estoque > 0
            WHERE produtos.id IN (
                SELECT produtos_ordem_catalogo.id_produto
                FROM produtos_ordem_catalogo
                WHERE produtos_ordem_catalogo.id BETWEEN $offset AND $offset + $porPagina - 1
            )
            $filtroWhereSQL
            GROUP BY produtos.id;";

            $retornoArray = $conexao->query($query)->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $query = "SELECT
                produtos.preco_promocao > 0 situacao_Promocao,
                produtos.preco_promocao = 0 situacao_Normal,
        ...
        $consulta = [...$consultaFoguinho, ...$retornoArray];

        return array_map(function ($item) {
            $item['situacoes'] = self::calculaSituacoesProdutoCatalogo($item);
            $item['foguinho'] = (bool) $item['foguinho'];
            return $item;
        }, $consulta);
    }
    ...
    public static function detalhesProdutos(
        PDO $conexao,
        array $transacoes,
        int $idResponsavelEstoque,
        int $idCliente
    ): array {
        $diasParaOCancelamento = ConfiguracaoService::buscaDiasDeCancelamentoAutomatico($conexao) + 1;
        $where = '';
        if (!empty($transacoes)) {
            [$bindKeys, $bind] = ConversorArray::criaBindValues($transacoes, 'id_transacao');
            $where .= " AND transacao_financeiras_produtos_itens.id_transacao IN ($bindKeys) ";
        }

        if ($idResponsavelEstoque != $idCliente) {
            $situacaoConferencia = LogisticaItem::SITUACAO_FINAL_PROCESSO_LOGISTICA;
            $where .= " AND logistica_item.situacao < $situacaoConferencia";
        }
        $bind[':id_responsavel_estoque'] = $idResponsavelEstoque;
        ...
        $sql = $conexao->prepare(
            "SELECT
                ...
                (
                    SELECT DATE_FORMAT(transacao_financeiras.data_atualizacao, '%d/%m/%Y %h:%i:%s')
                    FROM transacao_financeiras
                    WHERE transacao_financeiras.id = transacao_financeiras_produtos_itens.id_transacao
                ) data_compra,
                (
                    SELECT DATE_FORMAT(retorna_dia_util(DATE_ADD(transacao_financeiras.data_atualizacao, INTERVAL $diasParaOCancelamento DAY)), '%d/%m/%Y')
                    FROM transacao_financeiras
                    WHERE transacao_financeiras.id = transacao_financeiras_produtos_itens.id_transacao
                ) data_correcao,
                (
                ...
                colaboradores.razao_social consumidor_final,
                colaboradores.telefone,
                logistica_item.nome_tamanho
            FROM produtos
            INNER JOIN transacao_financeiras_produtos_itens ON transacao_financeiras_produtos_itens.id_produto = produtos.id
                AND transacao_financeiras_produtos_itens.id_responsavel_estoque = :id_responsavel_estoque
        ...
        foreach ($produtos as $key => $produto) {
            $produtos[$key]['qr_code'] = Globals::geraQRCODE(
                'https://api.whatsapp.com/send/?phone=55' . $produto['telefone']
            );
            unset($produtos[$key]['telefone']);
        }
