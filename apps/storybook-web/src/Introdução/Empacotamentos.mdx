...
- Para testar o pacote localmente (`npm pack --dry-run` na pasta `apps/storybook-[web ou native]/src/packages/<Seu Componente>`, com o [Verdaccio](#configurando-o-verdaccio) instalado e configurado, faça um `npm publish` e depois `yarn add @<escopo desejado>/<nome do pacote desejado>` no projeto onde deseja testar) para garantir que todas as dependências foram resolvidas corretamente e que não há problemas internos na lib empacotada.
...
- Comente todo o arquivo `.npmrc` na raiz do projeto  e também na raiz da sua máquina.
...
```
- Descomente o arquivo `.npmrc` na raiz do projeto exceto a linha 3.
...
- Para fazer a publicação automatica de todos os pacotes, siga os seguintes passos:
