...
- Para testar o pacote localmente (`npm pack --dry-run` na pasta `packages/<Seu Componente>`, com o [Verdaccio](https://verdaccio.org/) instalado e configurado, faça um `npm publish` e depois `yarn add @mobilestock/<nome do pacote desejado>` no projeto onde deseja testar) para garantir que todas as dependências foram resolvidas corretamente e que não há problemas internos na lib empacotada.
