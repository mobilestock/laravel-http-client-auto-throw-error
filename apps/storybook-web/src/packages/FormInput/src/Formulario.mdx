...
<Form ref={formRef} onSubmit={formulario}>
  <FormInput type="tel" name="nome" label="Nome" disabled value={usuarioSelecionado?.nome} />
  <FormInput autoFocus type="password" name="senha" label="Senha" required />
...
Os campos do formulário podem ser validados utilizando a biblioteca Yup. No exemplo acima, o campo "nome" é validado para garantir que não esteja vazio. Caso a validação falhe, a mensagem de erro é exibida.
