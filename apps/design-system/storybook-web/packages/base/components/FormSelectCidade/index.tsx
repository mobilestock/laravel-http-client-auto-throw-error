...
import { PropsCidadesRequest } from '../../contexts/useEndereco'
...
export const FormSelectCidade = ({
  ...
  const { fieldName, registerField, error } = useField(name ?? 'cidade')
  ...
  async function buscaCidadePorNome() {
    try {
      ...
      const { data } = await api.get<PropsCidadesRequest[]>(`api_administracao/cidades/pontos?${parametros}`)
      setResultado(data)
    } catch (error) {
      console.error(error)
    } finally {
  ...
  async function buscaCidadePorId() {
    try {
      ...
      setResultado([])
      const parametros = new URLSearchParams({
        pesquisa: defaultValue?.toString() ?? ''
      })
        ...
        <ClickAwayListener onClickAway={() => verificaVazio()}>
          <Autocomplete
            ...
            renderOption={(params, option) => {
              return (
                <Box component="li" key={params.id} {...params}>
